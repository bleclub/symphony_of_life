...
function startVideoPlayer(e) {
    if (e.video) {
        ...
        0 == e.showVideoControls && (a = [""]), jQuery(".instagram-element-video").mediaelementplayer({
        ...
        if (o.length > 0) {
            for (var r in o)
                if (a.limit < 20 && r >= a.limit);
                    ...
                    a.showDescription && s.caption && (l += ""), a.showLikes && (l += " <div class='likes-ui'><i aria-hidden='true' class='fa fa-heart-o'></i> " + s.likes.count + " Likes  </span>"), a.showComments && (l += " <span class='comments-ui'><i  aria-hidden='true' class='fa fa-comment-o'></i> " + s.comments.count + " Comments</span><br>"), a.showLocation && s.location && (l += "<br /><span class='location-ui'>Location: " + s.location.name.replace(/"/g, "'") + "</div>"), a.showLink && null != s.link && (l += "<a target='_blank' class='link-ui' href='" + s.link + "' style='text-decoration: underline; font-family: Helvetica;'> VIEW PHOTO</a>");
                    var c = "None";
                    s.videos && (c = s.videos.standard_resolution.url);
                    var d = a.element.selector.replace(".", "");
                    d = d.replace("#", ""), "video" == s.type && 1 == a.video ? (i = '<div class="brick"><a data-fancybox="cl-group" class="instagram-photo video" id="p' + s.id + '" href="#video-' + s.id + '" data-name="' + s.user.full_name + '" data-caption="' + l + '" data-created="' + s.created_time + '" data-author="' + s.user.username + '" data-likes="' + s.likes.count + '" data-comments="' + s.comments.count + '" data-video="' + c + '" data-profile="' + s.user.profile_picture + '" rel="group-' + d + '">', i += '<div>' + s.images.standard_resolution.url + '" </div>', i += '<span class="element-meta"><strong>' + s.user.full_name + "</strong><span>" + s.user.username + "</span></span>", i += '<span class="icon">Video</span>', i += '<div id="video-' + s.id + '" style="display:none; height:auto; width:640px;">', i += '<video class="instagram-element-video" width="100%" height="100%">', i += '<source src="' + c + '"></source>', i += "</video>", i += "</div>", i += "</a></div><table><tr><td>"+ s.user.full_name +"</td><td>"+ s.user.username +"</td><td>"+ s.link +"</td><td>"+ s.images.standard_resolution.url +"</td><td>VIDEO</td></tr></table>") : (i = '<div class="brick"><a data-fancybox="cl-group" class="instagram-photo image" id="p' + s.id + '" href="' + s.images.standard_resolution.url + '" data-name="' + s.user.full_name + '" data-caption="' + l + '" data-created="' + s.created_time + '" data-author="' + s.user.username + '" data-likes="' + s.likes.count + '" data-comments="' + s.comments.count + '" data-profile="' + s.user.profile_picture + '" rel="group-' + d + '">', i += '<div>' + s.images.standard_resolution.url + '</div>', i += '<span class="element-meta"><strong>' + s.user.full_name + "</strong><span>" + s.user.username + "</span></span>", i += '<span class="icon">Image</span>', i += "</a></div><table><tr><td>"+ s.user.full_name +"</td><td>"+ s.user.username +"</td><td>"+ s.link +"</td><td>"+ s.images.standard_resolution.url +"</td><td>PHOTO</td></tr></table>"), jQuery(i).appendTo(a.element)
